...
export default function App() {
  ...
  const classicPicks = getTitlesByCollection(titles, "clasicos").slice(0, 6);
  ...
  const appModel = {
    ...
    notification,
    notify(message) {
      // Cambio realizado para mejorar la accesibilidad en adultos mayores: confirma acciones importantes.
      setNotification(message);
      window.clearTimeout(window.__netflixSeniorToastTimer);
      window.__netflixSeniorToastTimer = window.setTimeout(() => {
        setNotification("");
      }, 3600);
    },
    getTitleById,
    ...
    toggleFavorite(id) {
      ...
      const willSave = !storedState.favorites.includes(id);
      ...
      // Cambio realizado para que la accion tenga retroalimentacion clara y no deje dudas.
      setNotification(
        willSave
          ? `${selectedTitle?.title ?? "Película"} guardada para ver después`
          : `${selectedTitle?.title ?? "Película"} quitada de guardados`,
      );
      window.clearTimeout(window.__netflixSeniorToastTimer);
      window.__netflixSeniorToastTimer = window.setTimeout(() => {
        setNotification("");
