...
const SIMPLE_COLLECTIONS = COLLECTIONS.filter((collection) =>
  ["clasicos", "historias-reales", "documentales", "espanol"].includes(collection.id),
);
...
export default function CategoriesScreen() {
  ...
  const activeCollection =
    ...
    SIMPLE_COLLECTIONS[0].id;
  const selectedCollection =
    SIMPLE_COLLECTIONS.find((collection) => collection.id === activeCollection) ??
    SIMPLE_COLLECTIONS[0];
  ...
  return (
    <>
      <section className="panel-surface">
        ...
        />
        {/* Cambio realizado para reducir categorias y evitar exceso de opciones. */}
        <OptionBar
          items={SIMPLE_COLLECTIONS}
